...
class Workshop extends Model
{
     ...
     */
    public $table = 'agus_tabler_workshop';
    ...
    public static function getCategoryOptions() {
        return [
            1 => 'In House Training ISO',
            2 => 'Workshop Akreditasi AUN-QA',
            3 => 'Workshop Pelatihan AMI',
            4 => 'Workshop Peningkatan Penjamin Mutu',
        ];
    }
    ...
    protected function uploadFileToGoogleDrive($file)
    {
        try {
            ...
            if (isset($result['fileId'])) {
                // Simpan nama file yang diupload ke database tanpa trigger afterSave
                $uploadedFileName = $result['fileName'] ?? $fileName;
                if (!preg_match('/\.pdf$/i', $uploadedFileName)) {
                    $uploadedFileName .= '.pdf';
                }

                // \Log::info('Saving file_name to database', [
                //     'file_name' => $uploadedFileName,
                //     'id' => $this->id
                // ]);

                // Gunakan DB query langsung untuk bypass events
                // \DB::table($this->table)
                //     ->where('id', $this->id)
                //     ->update(['file_name' => $uploadedFileName]);
                ...
                //     'saved_file_name' => $this->file_name,
                //     'category' => $this->category,
    ...
    protected function deleteLocalFileRelation($file)
    {
        try {
            ...
            // \Log::info('Deleting local file', ['filePath' => $filePath]);
