 ...
 * Model
 */
class Survei_kepuasan extends Model
{
     ...
     */
    public $table = 'agus_tabler_survei_kepuasan';
    ...
    protected function uploadFileToGoogleDrive($file)
    {
        try {
            ...
            $targetFolderId = $this->getTargetFolderId('ROOT_SURVEI_KEPUASAN');
            if (!$targetFolderId) {
                \Log::error('Survei Kepuasan: unable to resolve target folder for ' . $this->periode . '/' . $this->prodi);
                return;
            }
            ...
            if (isset($result['fileId'])) {
                ...
                \Log::warning('Survei Kepuasan upload response missing fileId', ['response' => $result]);
            }
        } catch (\Exception $e) {
            $this->isUploadingToGoogleDrive = false;
            \Log::error('Survei Kepuasan Google Drive upload failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
        }
    }
    ...
    protected function deleteOldFileIfExists()
    {
        ...
        $oldFolderId = $this->getTargetFolderId('ROOT_SURVEI_KEPUASAN');
