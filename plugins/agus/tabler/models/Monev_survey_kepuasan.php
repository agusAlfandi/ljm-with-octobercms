 ...
 * Model
 */
class Monev_survey_kepuasan extends Model
{
     ...
     */
    public $table = 'agus_tabler_monev_survei_kepuasan';
    ...
    protected function uploadFileToGoogleDrive($file)
    {
        try {
            ...
            $targetFolderId = $this->getTargetFolderId('ROOT_MONEV_SURVEI_KEPUASAN');
            if (!$targetFolderId) {
                \Log::error('Monev Survei Kepuasan: unable to resolve target folder for ' . $this->periode . '/' . $this->prodi);
                return;
            }
            ...
            if (isset($result['fileId'])) {
                ...
                \Cache::forget('gdrive_structure_' . md5(GoogleDriveReader::FOLDER_IDS['ROOT_MONEV_SURVEI_KEPUASAN']));

                $this->deleteLocalFileRelation($file, $filePath);
            } else {
                \Log::warning('Monev Survei Kepuasan upload response missing fileId', ['response' => $result]);
            }
        } catch (\Exception $e) {
            $this->isUploadingToGoogleDrive = false;
            \Log::error('Monev Survei Kepuasan Google Drive upload failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
        }
    }
    ...
    protected function deleteOldFileIfExists()
    {
        ...
        $oldFolderId = $this->getTargetFolderId('ROOT_MONEV_SURVEI_KEPUASAN');
    ...
    public function afterDelete()
    {
        if ($this->file_name) {
            try {
                ...
                \Log::error('Monev Survei Kepuasan Google Drive delete failed: ' . $e->getMessage());
            }
        }
    }
