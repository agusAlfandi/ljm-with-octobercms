/**
 * Auto-fill title field from uploaded PDF file name for AMI
 */
+function ($) { "use strict";

    $(document).ready(function() {
        console.log('AMI form auto-fill script loaded');
        ...
        $(document).on('upload.oc.fileupload', '[data-control="fileupload"]', function(e, fileInfo) {
            if (fileInfo && fileInfo.file && fileInfo.file.name) {
                ...
                var titleInput = $('input[name="Ami_documents[title]"]');
                if (titleInput.length === 0) {
                    titleInput = $('#Form-field-Ami_documents-title');
                }
                if (titleInput.length === 0) {
