...
class AddNamesToSurveiKepuasanTable extends Migration
{
    public function up()
    {
        Schema::table('agus_tabler_survei_kepuasan', function ($table) {
            if (!Schema::hasColumn('agus_tabler_survei_kepuasan', 'periode_name')) {
                $table->string('periode_name')->nullable();
            }
            if (!Schema::hasColumn('agus_tabler_survei_kepuasan', 'prodi_name')) {
