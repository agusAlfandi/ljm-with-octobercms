...
class Plugin extends PluginBase
{
    ...
    {
        try {
            // merge configuration for GoogleDriveReader cache TTL
            $configPath = __DIR__ . '/config/gdrive.php';
            if (file_exists($configPath)) {
                \Config::set('agus.tabler::gdrive', require $configPath);
            }

            // console command to warm/clear cache
            if (class_exists('Agus\Tabler\Console\GoogleDriveCache')) {
                $this->registerConsoleCommand('tabler.gdrive_cache', 'Agus\Tabler\Console\GoogleDriveCache');
            }
        } catch (\Throwable $e) {
            \Log::error('Agus.Tabler plugin register() failed: ' . $e->getMessage());
        }
    }
